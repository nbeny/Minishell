...
void	ft_env(t_exec *exe, t_env *e)
{
	...
	if (e != NULL && exe != NULL)
	{
		while (exe->cmd[i[0]])
		{
			if ((i[1] = ft_equal(exe->cmd[i[0]])))
			{
				s = ft_add_env(exe, s, i[0], i[1]);
				i[2] += 1;
			}
			else if (!ft_strncmp(exe->cmd[i[0]], "-i\0", 3))
			{
				s = ft_env_opt_i(s, i[2]);
				i[2] = 0;
				i[3] = 1;
			}
			else if (!ft_strncmp(exe->cmd[i[0]], "env\0", 4) &&	\
				(str = ft_path_istrue(&exe->cmd[i[0]], s)))
			{
				ft_execute_path(str, &(exe->cmd)[i[0]], s);
				ft_print_env(s, i[2], i[3], 0);
				return ;
			}
			i[0]++;
		}
		ft_print_env(s, i[2], i[3], 1);
	}
}
